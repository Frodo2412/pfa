...
fromToTree b e = unfoldTree (\(v,t) -> if v == t 
                                then Left v 
                                else Right ((v,if v > t then v - div (v-t) 2 else div (t-v) 2 + v),
                                    (if v > t then v - 1 - div (v-t) 2 else div (t-v) 2 + v + 1,t))) (b,e)

mapTree :: (a -> b) -> Tree a -> Tree b
mapTree f = unfoldTree fTree
    where fTree (Leaf x)  = Left (f x)
